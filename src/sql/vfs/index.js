...
// });
export async function fileImport (event)  {
  console.log({event});
  let vfs;
  try {
    log(`Importing to IndexedDB ${IDB_NAME}, path ${DB_NAME}`);
...
//   }
// });
// document.getElementById("select").addEventListener("click",async(event)=>{
//   await select()
// })
async function select(){
  ...
  console.log({db});
  await sqlite3.exec(db, `SELECT * FROM words WHERE LIMIT 10;`, (row) => {
    console.log( `SELECT * FROM  ${DB_NAME} LIMIT 10;`,{row});
  });
